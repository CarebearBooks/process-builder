...
const TYPE_CONFIG: Record<string, {
  color: string; lightColor: string; label: string
...
export default function StepCard({ step, isDark, dragHandleProps }: Props) {
  ...
  function commitName() { updateStep(step.id, { name: nameVal }); setEditingName(false) }
  function commitDesc() { updateStep(step.id, { description: descVal }); setEditingDesc(false) }

  return (
    <div
      className="group w-full rounded-xl overflow-hidden cursor-pointer transition-all duration-150"
      style={{
        background: cardBg,
        border:     `1.5px solid ${isSelected ? tc.color : borderBase}`,
    ...
    >
      ...
      <div className="flex items-center gap-2 px-3 py-2" style={{ background: headerBg }}>
        <div
          {...dragHandleProps}
          className="flex items-center cursor-grab active:cursor-grabbing touch-none shrink-0"
          style={{ color: textMuted }}
          onClick={(e) => e.stopPropagation()}
        >
          <GripVertical className="h-3.5 w-3.5" />
        </div>
        <div className="h-2 w-2 rounded-sm shrink-0" style={{ background: typeColor }} />
        <span
          className="text-[10px] font-bold tracking-widest uppercase flex-1 truncate"
          style={{ color: typeColor }}
        >
          {tc.label}
        </span>
        {/* X — group is on the outer card div, so this works on hover */}
        <button
          className="flex h-5 w-5 items-center justify-center rounded shrink-0 opacity-0 group-hover:opacity-100 transition-opacity duration-150"
          style={{ color: isDark ? 'rgba(255,255,255,0.4)' : 'rgba(0,0,0,0.35)' }}
          onClick={(e) => { e.stopPropagation(); deleteStep(step.id) }}
          onMouseEnter={(e) => (e.currentTarget.style.color = '#f43f5e')}
          onMouseLeave={(e) => (e.currentTarget.style.color = isDark ? 'rgba(255,255,255,0.4)' : 'rgba(0,0,0,0.35)')}
        >
      ...
      <div className="px-3 py-2.5">
        {editingName ? (
          <input
            ...
            onBlur={commitName}
            onKeyDown={(e) => { if (e.key === 'Enter') commitName(); if (e.key === 'Escape') { setNameVal(step.name); setEditingName(false) } }}
            onClick={(e) => e.stopPropagation()}
            className="w-full bg-transparent text-sm font-semibold outline-none border-b mb-0.5"
            style={{ color: textMain, borderColor: typeColor }}
          />
        ) : (
          <p
            className="text-sm font-semibold mb-0.5 truncate select-none"
        ...
        {editingDesc ? (
          <input
            ...
            onBlur={commitDesc}
            onKeyDown={(e) => { if (e.key === 'Enter') commitDesc(); if (e.key === 'Escape') { setDescVal(step.description ?? ''); setEditingDesc(false) } }}
            onClick={(e) => e.stopPropagation()}
        ...
        <div className="flex items-center gap-2 mt-2.5">
          <div
            ...
            style={{ height: '3px', background: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.1)' }}
          >
            <div
          ...
          <span className="text-[10px] shrink-0" style={{ color: textMuted }}>{ac.label}</span>
        </div>
      </div>
    </div>
  )
